...
class CustomDatasetFromImagesTemporal(SatelliteDataset):
    def __init__(self, csv_path: str):
        ...
        def normalize_name(n):
            name_part, ext = os.path.splitext(n)
            if name_part.endswith('_crop_0'):
                return name_part[:-len('_crop_0')] + ext
            return n
        ...
        self.data_info = pd.read_csv(csv_path, header=0)
        # First column contains the image paths
        self.image_arr = np.array([normalize_name(n) for n in np.asarray(self.data_info.iloc[:, 1])])
        # Second column is the labels
        self.label_arr = np.asarray(self.data_info.iloc[:, 0])
        # Calculate len
...
class SentinelIndividualImageDataset(SatelliteDataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            print(f"Warning: {selection['image_path']} not found in sentinel_files")
            dummy_image = torch.zeros((self.in_c, 96, 96), dtype=torch.float32)
            return dummy_image, self.categories.index(selection['category'])
