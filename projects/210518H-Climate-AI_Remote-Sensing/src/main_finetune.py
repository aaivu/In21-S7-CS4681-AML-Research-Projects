...
def main(args):
    ...
    if args.finetune and not args.eval:
        ...
        if 'patch_embed.proj.weight' in checkpoint_model:
            ckpt_weight = checkpoint_model['patch_embed.proj.weight']
            model_weight = state_dict['patch_embed.proj.weight']

            if ckpt_weight.shape != model_weight.shape:
                print(f"Adapting 'patch_embed.proj.weight' from {ckpt_weight.shape} to {model_weight.shape}")
                if ckpt_weight.shape[1] < model_weight.shape[1]:
                    # Initialize new weights with zeros and copy what we can
                    ...
                    # If ckpt has more channels, truncate
                    checkpoint_model['patch_embed.proj.weight'] = ckpt_weight[:, :model_weight.shape[1], :, :]
        ...
        if unexpected_missing:
            ...
            print("✅ Missing keys are as expected.")
