# Research Proposal: Impact of Activation Functions on PEGASUS-X for Abstractive Text Summarization
...
This research investigates architectural enhancements to the PEGASUS-X transformer model for improved long document summarization. Despite PEGASUS-X's state-of-the-art performance on lengthy texts, opportunities exist for optimization through targeted modifications that maintain computational efficiency. The study proposes two key architectural changes 
- Replacing standard GeLU activation functions with other activation functions to improve gradient flow and training stability
- Introducing lightweight intermediate layers between attention and feed-forward blocks to enhance representational capacity without significant parameter overhead.

The research methodology involves fine-tuning modified PEGASUS-X variants on diverse datasets, with input lengths ranging from 430 to 10,000 tokens. Performance evaluation employs ROUGE metrics to assess summarization quality improvements over the baseline model. This work contributes empirical insights into activation function selection for long-sequence transformers and demonstrates practical approaches for enhancing existing summarization architectures through minimal but strategic modifications.
...
### Efficient Attention Mechanisms for Long Inputs
...
### Hierarchical and Multi-stage Summarization
...
Karotia and Susan have tackled lay language summarisation by first fine tuning general models on domain specific data, and then feeding their outputs into a specialized domain specific model for further fine tuning. They have found that combining general and domain specific summarizers improves performance across relevance, readability and factuality metrics.
...
### Baseline Model - PEGASUS-X
PEGASUS-X is an extension of the original PEGASUS summarisation model, which was designed to handle long input sequences of up to 16,384 tokens while remaining efficient in terms of memory and computation. The model is based on an encoder–decoder architecture, but introduces several key modifications.
...
### Datasets
The datasets used for this project are summarised in the table below. Long document benchmarks are the primary focus, while short document corpora are included for comparison to ensure modifications do not degrade performance on shorter inputs.
...
- **Improved summarisation quality -** The use of alternative activation functions and architectural adjustments are anticipated to improve ROUGE scores compared to the baseline PEGASUS-X model.
...
- **Empirical comparison of activations -** The evaluation will clarify whether other activation functions yield better performance for long document summarisation tasks, contributing to understanding activation choice in transformer models.
...
- **Datasets -** arXiv, PubMed, GovReport, BigPatent, CNN/DailyMail, XSum 
