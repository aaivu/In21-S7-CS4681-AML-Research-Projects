"""
RoBERTa-based Multi-Task Model for Toxicity Classification
"""
...
class RoBERTaForToxicity(nn.Module):
    """
    RoBERTa-base model for multi-task toxicity classification.
    ...
    def __init__(self, model_name='roberta-base', dropout_rate=0.1):
        """
        Initialize the RoBERTa toxicity model.
        
        Args:
            model_name (str): HuggingFace model identifier
                            Options: 'roberta-base' (125M params) or 'roberta-large' (355M params)
            dropout_rate (float): Dropout probability for classifier head
        """
        super(RoBERTaForToxicity, self).__init__()
    ...
    def freeze_backbone(self):
        """Freeze RoBERTa parameters for feature extraction only."""
        for param in self.roberta.parameters():
            param.requires_grad = False
    
    def unfreeze_backbone(self):
        """Unfreeze RoBERTa parameters for fine-tuning."""
        for param in self.roberta.parameters():
