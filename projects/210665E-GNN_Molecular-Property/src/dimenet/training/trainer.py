...
class Trainer:
    def __init__(self, model, learning_rate=1e-3, warmup_steps=None,
                 ...
                 ema_decay=0.999, max_grad_norm=10.0, weight_decay=1e-6):
        self.model = model
        ...
        # Use standard Adam optimizer to avoid tensorflow-addons dependency
    ...
    def load_averaged_variables(self):
        # No-op when not using moving average optimizer
        pass
    ...
    def restore_variable_backups(self):
        # No backups created when not using EMA; safely no-op
        if self.backup_vars is None:
            return
        for var, bck in zip(self.model.trainable_weights, self.backup_vars):
            var.assign(bck)
    ...
    def train_on_batch(self, dataset_iter, metrics):
        ...
        with tf.GradientTape() as tape:
            ...
            loss = mean_mae
            # Small L2 regularization to ensure all parameters receive gradients
            if self.weight_decay is not None and self.weight_decay > 0.0:
                l2 = tf.add_n([tf.nn.l2_loss(v) for v in self.model.trainable_weights])
