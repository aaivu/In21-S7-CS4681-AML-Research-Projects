...
class BesselBasisLayer(layers.Layer):
    def __init__(self, num_radial, cutoff, envelope_exponent=5,
        ...
        def freq_init(shape, dtype=None):
            # shape may be tuple/list in newer Keras; take first dim
            n = shape[0] if isinstance(shape, (tuple, list)) else int(shape)
            vals = np.pi * np.arange(1, n + 1, dtype=np.float32)
            return tf.constant(vals, dtype=tf.float32 if dtype is None else dtype)
