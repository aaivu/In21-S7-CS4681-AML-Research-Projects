...
eval_config = Task2Config(
    model_name="Chamika-Jaya/Llama-3.2-3B-Instruct-Fine_tuned",
...
for date in tqdm(eval_config.eval_dates, desc="Evaluating..."):
    ...
    for ticker in eval_config.tickers:
        ...
        close_price = prices.loc[
            prices["Ticker"] == ticker, "Close"
        ].item()  # buy at today's close
        future_price = prices.loc[
            prices["Ticker"] == ticker, "future_close"
        ].item()  # sell at future close
        ...
        if signal_score >= eval_config.threshold:
            ...
            value_change = -1 * (
                (future_price - close_price) / close_price
            )  # good if negative % change
        else:
            value_change = 0

        logging_cum_returns_df_threshold_based.append(
            {"Date": date, "Ticker": ticker, "ValueChange": value_change}
        )

    ### EVAL STRATEGY ###
    sorted_tickers = sorted(ticker_signals.items(), key=lambda x: x[1], reverse=True)
    sorted_thresh_tickers_long = [
        ticker for ticker in sorted_tickers if ticker[1] > eval_config.threshold
    ]
    sorted_thresh_tickers_short = [
        ticker for ticker in sorted_tickers if ticker[1] < -eval_config.threshold
    ]
    long_tickers = sorted_thresh_tickers_long[
        : eval_config.num_long
    ]  # Highest signal scores
    short_tickers = sorted_thresh_tickers_short[
        eval_config.num_short :
    ]  # Lowest signal scores
    ...
    eval_cum_returns_data.append(
        {
            "Date": date,
            "MeanEvalReturn": np.mean(eval_rets),
            "win_loss": value_change >= 0,
        }
    )
...
### LOGGING -
# Create DataFrames
logging_cum_returns_df_threshold_based = pd.DataFrame(
    logging_cum_returns_df_threshold_based
)
logging_cum_returns_df_threshold_based["CumulativeReturn"] = (
    logging_cum_returns_df_threshold_based.groupby("Ticker")["ValueChange"].cumsum()
)

# calculate cumulative returns and win loss
eval_cum_returns_df = pd.DataFrame(eval_cum_returns_data)
eval_cum_returns_df["cumulative_return"] = (
    1 + eval_cum_returns_df["MeanEvalReturn"]
).cumprod() - 1
...
logging_cum_returns_df_threshold_based.to_csv(
    "task2_threshold_based_cumulative_returns.csv", index=False
)
...
for ticker in STOCK_TICKERS_HIGHEST_CAP_US:
    ticker_data = logging_cum_returns_df_threshold_based[
        logging_cum_returns_df_threshold_based["Ticker"] == ticker
    ]
    axes[0].plot(
        ticker_data["Date"], np.log1p(ticker_data["CumulativeReturn"]), label=ticker
    )
...
axes[1].plot(
    eval_cum_returns_df["Date"],
    eval_cum_returns_df["MeanEvalReturn"],
    color="blue",
    marker="o",
)
axes[1].plot(
