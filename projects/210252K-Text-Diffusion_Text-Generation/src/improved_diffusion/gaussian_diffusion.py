...
from .losses import rounding_losses
from . import losses as custom_losses  # at top of file if not already imported
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.lambda_qce = lambda_qce
        self.rounding_tau = rounding_tau
    ...
    def training_losses_emb(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if self.model_var_type in [
                ...
                if hasattr(self, "lambda_anchor") and self.lambda_anchor > 0:
                    emb_matrix = model.model.module.get_embeds.weight  # vocab embeddings
                    L_anchor, L_qce = custom_losses.rounding_losses(
                        hat_x0=terms.get("pred_xstart", model_output),  # x0 predictions
                        tokens=input_ids,
                        emb_matrix=emb_matrix,
                        tau=getattr(self, "rounding_tau", 0.1)
                    )
                    terms["anchor_loss"] = L_anchor
                    terms["qce_loss"] = L_qce
                    terms["loss"] = terms["loss"] + self.lambda_anchor * L_anchor + self.lambda_qce * L_qce
