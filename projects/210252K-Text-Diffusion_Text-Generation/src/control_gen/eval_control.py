    ...
    # # import evaluate
    # import evaluate as parser_evaluate
    ...
    #         ).cuda()
    #     except:
    #         print(f"Could not load {args.model_name_or_path}, using gpt2 instead")
    ...
    #     #     # print(pred_trees[0])
    #     #     dev_fscore = parser_evaluate.evalb('diffusion_lm/misc/self-attentive-parser/EVALB',
...
#         model = AutoModelForCausalLM.from_pretrained("gpt2").cuda()
#         args.model_name_or_path = "gpt2"  # Update for tokenizer loading
...
# Handle both list (from 'file' format) and dict (from 'paired' format)
    if isinstance(text_samples, dict):
        # Paired format: iterate over dictionary
        samples_to_process = text_samples.items()
    else:
        # File format: convert list to dict-like structure
        samples_to_process = [(i, [sample]) for i, sample in enumerate(text_samples)]

    for gold, full_word_lst in samples_to_process:
        agg_loss = []
        for x in full_word_lst:
            # Simply join the tokens without diffusion tokenizer filtering
            x = tokenizer.bos_token + " ".join(x) + tokenizer.eos_token

            tokenized_x = tokenizer(x, return_tensors='pt')
            input_ids = tokenized_x['input_ids'].cuda()
            labels = input_ids.clone()
            model_output = model(input_ids, labels=labels)
            agg_loss.append(model_output.loss.item())
...
def _normalize_sent(s):
    """Normalize sentence to prevent tokenization issues"""
    if hasattr(s, 'words'):
        s = ' '.join(s.words)
    elif isinstance(s, (list, tuple)):
        s = ' '.join(str(w) for w in s)
    # Clean up the string
    s = s.replace("\xa0", " ")
    s = " ".join(s.split())
    return s.strip()
...
def eval_parse(parser, generated, tree_vocab):
    ...
    for sent in generated:
        ...
        normalized = _normalize_sent(sent)
        
        # Parse as string instead of InputSentence to avoid tokenization issues
        sent_lst.append(normalized)
    
    # Parse the normalized strings directly
    try:
        parse_lst = list(parser.parse_sents(sent_lst))
    except (AssertionError, StopIteration) as e:
        print(f"Parsing error: {e}")
        print("Attempting to parse individual sentences...")
        parse_lst = []
        for i, sent in enumerate(sent_lst):
            try:
                parsed = list(parser.parse_sents([sent]))[0]
                parse_lst.append(parsed)
            except Exception as e2:
                print(f"Failed to parse sentence {i}: {sent}")
                print(f"Error: {e2}")
                # Create a dummy parse tree as fallback
                from nltk.tree import Tree
                words = sent.split()
                dummy_tree = Tree('S', [Tree('NN', [w]) for w in words])
                parse_lst.append(dummy_tree)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_text', type=str, 
                    default='diffusion_lm/improved_diffusion/out_gen/diff_e2e-tgt_pad_rand16_transformer_lr0.0001_0.0_2000_sqrt_Lsimple_h128_s2_d0.1_sd102_xstart.ema_0.9999_200000.pt.infill_control_tree_50x64x16_tree_partial-cat-lgv0.1.json',
                    help='Path to input file')
    parser.add_argument('--input_format', type=str, default='paired', 
                    help='Input format: "file" or "paired"')
    parser.add_argument('--mode', type=str, default='e2e-tgt-tree-paired', 
                    help='Evaluation mode: e2e-tgt-tree, e2e-tgt-pos, e2e-tgt-tree-paired, e2e-tgt-pos-paired, e2e-tgt-spans-paired, e2e-tgt-attribute-paired, e2e-tgt-length-paired')
    parser.add_argument('--gold_ref', type=str, default='full', 
                    help='Gold reference type: "full" or "span"')
    parser.add_argument('--model_name_or_path', type=str, default='gpt2', 
                    help='Model name or path for perplexity evaluation (default: gpt2)')
    # parser.add_argument('--model_name_or_path', type=str, 
    #                default='/finetuned_gpt2_e2e/finetuned_gpt2_e2e', 
    #                help='Model name or path for perplexity evaluation (default: gpt2)')
