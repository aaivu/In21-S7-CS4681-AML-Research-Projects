...
from trainer.trainer_multiscale_egoclip import MultiScale_Trainer_dist
from utils.util import replace_nested_dict_item
from tensorboardX import SummaryWriter

ex = Experiment('train_multiscale')
...
def run():
    ...
    # Initialize temperature scheduler if specified in config
    temp_scheduler = None
    if 'temperature_scheduler' in config['trainer']:
        temp_scheduler_config = config['trainer']['temperature_scheduler']
        temp_scheduler = module_loss.CosineTemperatureScheduler(
            initial_temp=temp_scheduler_config['initial_temp'],
            final_temp=temp_scheduler_config['final_temp'],
            total_steps=temp_scheduler_config['total_steps']
        )
        # Set scheduler in loss function if it's MultiScaleEgoNCE
        if hasattr(loss, 'temp_scheduler'):
            loss.temp_scheduler = temp_scheduler
    ...
    # Create enhanced trainer with temperature scheduler
    trainer = MultiScale_Trainer_dist(args, model, loss, metrics, optimizer,
                      config=config,
...
if __name__ == '__main__':
    ...
    args = argparse.ArgumentParser(description='PyTorch Multi-Scale EgoVLP Training')
    args.add_argument('-c', '--config', default='configs/pt/multiscale_egoclip.json', type=str,
    ...
    args.add_argument('-lr1', '--learning_rate1', type=float, default=2e-5)
    ...
    options = [
        ...
        CustomArgs(['--cw', '--consistency_weight'], type=float, target=('loss', 'args', 'consistency_weight')),
    ]
