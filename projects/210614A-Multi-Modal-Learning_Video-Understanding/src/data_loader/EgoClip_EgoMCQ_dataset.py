...
class EgoClip_EgoMCQ(TextVideoDataset):
    ...
    def _get_video_multiscale(self, video_fp, video_sec, bound_sec):
        """
        Sample frames at 3 different densities from the same temporal span.
        
        Args:
            video_fp: Video file paths
            video_sec: [start_sec, end_sec] temporal boundaries
            bound_sec: Boundary second for chunk handling
            
        Returns:
            Dictionary: {'frames_4': tensor, 'frames_8': tensor, 'frames_16': tensor}
        """
        video_loading = self.video_params.get('loading', 'strict')
        
        # Calculate clip duration
        clip_duration = video_sec[1] - video_sec[0]
        
        # Define frame counts for each scale
        scales = [4, 8, 16]
        result = {}
        ...
        for num_frames in scales:
            try:
                if os.path.isfile(video_fp[0]) and os.path.isfile(video_fp[1]):
                    # For clips shorter than 1s, handle differently based on scale
                    if clip_duration < 1.0 and num_frames == 16:
                        # For very short clips, pad with boundary frames for coarse scale
                        imgs, idxs = self.video_reader(video_fp[0], video_fp[1], min(num_frames, 8), 
                                                     'uniform', start_sec=video_sec[0], end_sec=video_sec[1], 
                                                     bound_sec=bound_sec)
                    else:
                        # Uniformly sample frames for this scale
                        imgs, idxs = self.video_reader(video_fp[0], video_fp[1], num_frames, 
                                                     'uniform', start_sec=video_sec[0], end_sec=video_sec[1], 
                                                     bound_sec=bound_sec)
                else:
                    print(f"Warning: missing video file {video_fp}.")
            ...
            # Apply transforms (maintain existing preprocessing)
            if self.transforms is not None:
                if num_frames > 1:
                    imgs = imgs.transpose(0, 1)  # [T, C, H, W] ---> [C, T, H, W]
                    imgs = self.transforms(imgs)
            ...
            # Create final tensor with proper padding
            final = torch.zeros([num_frames, 3, self.video_params['input_res'],
                               self.video_params['input_res']])
            
            if imgs.shape[0] > 0:
                # Handle cases where we have fewer frames than requested
                available_frames = min(imgs.shape[0], num_frames)
                final[:available_frames] = imgs[:available_frames]
                
                # For clips shorter than needed frames, pad with boundary frames
                if imgs.shape[0] < num_frames:
                    if imgs.shape[0] > 0:
                        # Repeat last frame to fill remaining slots
                        final[imgs.shape[0]:] = imgs[-1].unsqueeze(0).repeat(num_frames - imgs.shape[0], 1, 1, 1)
            
            result[f'frames_{num_frames}'] = final
