...
where \( \mathcal{D} \) is the learned diffusion operator, and \( \epsilon_t \) is the perturbation noise. The goal is to design \( \Sigma \) such that it reflects realistic atmospheric variability, informed by geographic and physical constraints.
