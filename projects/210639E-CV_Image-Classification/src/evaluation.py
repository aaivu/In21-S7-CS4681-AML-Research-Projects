...
def main(args):
    ...
    # If user requested AMP but selected CPU, disable AMP to avoid GradScaler warning
    # if device.type == 'cpu' and args.amp:
    #     print("AMP requested but CUDA is not available; disabling AMP for CPU run.")
    #     args.amp = False
    ...
    if args.model_ema:
        ...
        print("EMA model tracking enabled.")
    ...
    # Only create a GradScaler / NativeScaler when AMP is enabled (CUDA available)
    ...
    if args.resume:
        ...
        # Remove classifier weights if shape mismatch (ImageNet vs CIFAR100)
        if 'head.weight' in state_dict and state_dict['head.weight'].shape[0] != args.nb_classes:
            del state_dict['head.weight']
            del state_dict['head.bias']
            print("Classifier head removed from checkpoint (mismatch). New head initialized.")

        msg = model_without_ddp.load_state_dict(state_dict, strict=False)
        print("Loaded checkpoint with msg:", msg)
    ...
    for epoch in range(args.epochs):
        ...
        # simple sequential loader for training
        train_loader = torch.utils.data.DataLoader(
            dataset_train, batch_size=args.batch_size,
            shuffle=True, num_workers=args.num_workers, pin_memory=args.pin_mem, drop_last=True
        )

        # Initialize the tqdm progress bar
        with tqdm(enumerate(train_loader), total=len(train_loader), desc=f"Epoch {epoch+1}/{args.epochs}") as pbar:
            for batch_idx, (images, targets) in pbar:
                ...
                # Track loss and accuracy (but don't print batch-wise details)
        ...
        print(f"Epoch [{epoch+1}/{args.epochs}]  Loss: {avg_loss:.4f}  Train Acc: {train_acc:.2f}%")

        lr_scheduler.step(epoch)
        # === Save model checkpoint ===
        if not args.eval:
            resume_name = Path(args.resume).stem if args.resume else "new"
            # Safely get last epoch number if available
            last_epoch = checkpoint['epoch'] if args.resume and 'epoch' in checkpoint else 0
            current_epoch = last_epoch + (epoch + 1)

            save_name = f"{resume_name}_finetuned_epoch{current_epoch}.pth"
