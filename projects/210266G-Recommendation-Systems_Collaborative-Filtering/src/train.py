...
def main(args):
    ...
    experiment_name = f"LightGCN_dim{args.embed_dim}_layers{args.n_layers}_lr{args.lr}_reg{args.reg}_{timestamp}"
    results_path = os.path.join('results/lightgcn_logs', experiment_name)
    ...
    best_recall = -1.0 # Initialize to a negative value
    ...
    for epoch in range(args.epochs):
        ...
        for users, pos_items, neg_items in tqdm(dataloader, desc=f"Epoch {epoch+1}/{args.epochs}"):
            ...
            optimizer.zero_grad()
            users_emb, pos_items_emb, neg_items_emb = model(users, pos_items, neg_items)
            loss = bpr_loss(users_emb, pos_items_emb, neg_items_emb, args.reg)
            loss.backward()
            optimizer.step()
        ...
        # --- Validation ---
        # --- KEY FIX 1: Evaluate on the last epoch OR every N epochs ---
    ...
    log_and_print("\n--- Final Test Evaluation ---")
    # --- KEY FIX 2: Check if the model file exists before loading ---
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='LightGCN Baseline Training')
    parser.add_argument('--data_path', type=str, default='data', help='Dataset directory relative to project root')
    parser.add_argument('--epochs', type=int, default=10, help='Number of epochs') # Increased default
